...
/* helper function to get/set attributes if set is 0 it will be a get, otherwise is is interpreted
 * as set */
...
int mlx90394_trigger_measurement_internal(const struct device *dev, enum sensor_channel chan)
{
	...
	/* change channel bits and update ctrl4 if neccessary only if the channel is another
...
{
	/* TODO check that the cycle time fitts to the conversion time */
	...
	/* adding 5% tollerance from datasheet infact that this applys to the clock
	   of 2.4MHz this should be true global */
...
static int mlx90394_init(const struct device *dev)
{
	...
	if ((rc = mlx90394_write_read_dt(&cfg->i2c, MLX90394_REG_CTRL1,
					 (uint8_t *)&data->ctrl_reg_values.ctrl1,
					 (uint8_t *)&data->ctrl_reg_values.ctrl1, 2)) != 0) {
		return rc;
	}
	if ((rc = mlx90394_write_read_dt(&cfg->i2c, MLX90394_REG_CTRL3,
					 (uint8_t *)&data->ctrl_reg_values.ctrl3,
					 (uint8_t *)&data->ctrl_reg_values.ctrl3, 2)) != 0) {
		return rc;
	}
