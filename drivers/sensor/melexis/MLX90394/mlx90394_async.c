...
void mlx90394_async_fetch(struct k_work *work)
{
	...
	rc = mlx90394_sample_fetch_internal(dev, *cfg->channels);
	if (rc != 0) {
	...
	default: {
		LOG_DBG("Invalid channel %d", *cfg->channels);
...
int mlx90394_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	int rc;
	const struct sensor_read_config *cfg = iodev_sqe->sqe.iodev->data;
	struct mlx90394_data *data = dev->data;

	rc = mlx90394_trigger_measurement_internal(dev, *cfg->channels);
	if (rc != 0) {
